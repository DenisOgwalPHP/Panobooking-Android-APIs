...
if (isset($_POST)) {
    ...
    $sql = "INSERT into cart(ProductName,Quantity,Cost,User,OrderCode,DatesFrom,DatesTo,Descriptions,Facility,ImageUrl,BookingType,Currency,CurrencyAmount)VALUES('$product','1','$cost','$users','$ordercode','$datesfrom','$datesto','$desDesc','$Facilityname','$product','Tour Package','$Currency','$CurrencyAmount')";
    ...
    if ($result) {
        ...
        if (mysqli_num_rows($result8) > 0) {
            while ($row2 = mysqli_fetch_array($result8)) {
                ...
                $sql7 = "SELECT EmailAddress FROM tourpackage  where PackageName  like '" . $product . "%' and TravelCompany  like '" . $Facilityname . "%'";
                ...
                if (mysqli_num_rows($result7) > 0) {
                    while ($row = mysqli_fetch_array($result7)) {
                        ...
                        $to = trim($rowresult1);
                        $booknotes = 'Please Reserve for ' . $rowresult2 . ' ' . $product . '. TOTAL PAYABLE: ' . $Currency . ' ' . $cost . ', Booking Notes: ' . $desDesc;
                        ...
                        $message .= '<div style="padding:10px;text-align:center"><a href="https://tourpackages.panobooking.com/"><button style="padding:10px;text-align:center;height:50px;border-radius: 10px;background-color:#3383ff; color:white;font-size:20px;">Confirm Order</button></a></div></div>';
                        $message .= '</body></html>';
