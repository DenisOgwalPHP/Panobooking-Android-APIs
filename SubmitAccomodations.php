...
if (isset($_POST)) {
    ...
    $sql = "INSERT into cart(ProductName,Quantity,Cost,User,OrderCode,DatesFrom,DatesTo,Descriptions,Facility,ImageUrl,BookingType,Currency,CurrencyAmount)VALUES('$product','1','$cost','$users','$ordercode','$datesfrom','$datesto','$desDesc','$Facilityname','Hotel','Stays','$Currency','$CurrencyAmount')";
    $result = mysqli_query($link, $sql);
    $response = array();
    if ($result) {
        $sql1 = "UPDATE rooms SET Availability='Available' WHERE RoomName='" . $product . "' and HotelName='" . $Facilityname . "'";
        ...
        if (mysqli_num_rows($result8) > 0) {
            while ($row2 = mysqli_fetch_array($result8)) {
                ...
                $sql7 = "SELECT EmailAddress FROM pickuplocations  where Place  like '" . $Facilityname . "%'";
                ...
                if (mysqli_num_rows($result7) > 0) {
                    while ($row = mysqli_fetch_array($result7)) {
                        ...
                        $to = trim($rowresult1);
                        $booknotes = 'Please Reserve for ' . $rowresult2 . ' 1 ' . $product . ' Rooms for a Period: From ' . $datesfrom . '. To: ' . $datesto . '. TOTAL PAYABLE: ' . $cost . ', Booking Notes: ' . $desDesc;
                        ...
                        $message .= '<div style="padding:10px;text-align:center"><a href="https://hotel.panobooking.com/"><button style="padding:10px;text-align:center;height:50px;border-radius: 10px;background-color:#3383ff; color:white;font-size:20px;">Confirm Order</button></a></div></div>';
                        $message .= '</body></html>';
